...
导出为 Sub2API 时，账号名称使用完整邮箱（依次读取 `credentials.email`、`email`、`extra.email`），仅去除首尾空白。有邮箱时覆盖原名称；无邮箱时保留已有 Sub2API 名称，其他格式转换继续使用序号名称。CPA 和原始结构输出保持原有字段，下载文件名继续采用下方的中性命名规则。
